...
function getNodeDataFromHTML() {
  return Array.from(document.querySelectorAll("#node-data > div")).map(
    (div) => ({
      ...
      angle: Math.random() * 2 * Math.PI, // Инициализация угла для каждого узла
      radius: (Math.random() + 0.4) * 300, // Радиус вращения узлов
    })
  );
}
...
const simulation = d3
  ...
  .force("charge", d3.forceManyBody().strength(-1000)) // Ослабление отталкивания между узлами
...
const node = svg
  ...
  .join("circle")
  .attr("r", (d) => (d.label === "Ruscom" ? 30 : 20)) // Изменение размера узла
  .attr("fill", "#9f9f9f")
